...
('users-test-admin@example.invalid','admin'),('users-test-operator@example.invalid','technician');
...
do $$ begin
  begin
    perform public.manage_service_users('test-only-password','add','users-test-illegal@example.invalid',null,'user');
...
do $$ declare result jsonb; begin
  ...
  result:=public.manage_service_users('test-only-password','add',' USERS-TEST-NEW@EXAMPLE.INVALID ',null,'supervisor');
  assert exists(select 1 from jsonb_array_elements(result->'users') u where u->>'email'='users-test-new@example.invalid' and u->>'role'='supervisor' and (u->>'active')::boolean),'normalized supervisor added';
  assert public.manage_service_users('test-only-password','add','users-test-new@example.invalid',null,'user') ? 'error','duplicate rejected';
  ...
  result:=public.manage_service_users('test-only-password','set_role','users-test-new@example.invalid',null,'technician');
  assert exists(select 1 from jsonb_array_elements(result->'users') u where u->>'email'='users-test-new@example.invalid' and u->>'role'='technician'),'role changed';
  assert public.manage_service_users('test-only-password','set_role','users-test-new@example.invalid',null,'admin') ? 'error','cannot create another admin';
...
reset role;
do $$ begin
  assert (select count(*) from private.service_access_audit where target_email='users-test-new@example.invalid')=4,'audit for add, toggles and role change';
